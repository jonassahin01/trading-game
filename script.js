...
const assets = [
  ...
  "WBC",
  "WWW",
]; // 100 possible underlying assets
...
let dataFlowIntervalTimer = Math.round(
  (10 / $("#data-speed-input").val()) * 1000
); // The intervals (in milliseconds) on how often to execute the dataFlowIntervalFunction (min: 0.5s, max: 10s)
...
let gameHistory = {
  // Holds the game history and current state
...
$("#start-end-game").click(function (event) {
  if (isGameInProgress) {
    ...
    if (remainingDataPoints.length > 0) {
      if (gameHistory.history.length > 0) {
        clearPreviousGame();
      }
...
$("#play-pause").click(function (event) {
  if (isGamePaused) {
    ...
    dataFlowIntervalFunction = setInterval(
      () => addDataPoint(),
      dataFlowIntervalTimer
    );
  } else {
    // Pause data flow
    $("#play-pause i")
...
$("#buy-sell-buttons button").click(function (event) {
  if (!isGameInProgress) {
    ...
    setTimeout(function () {
      $("#start-end-game").removeClass("tada");
    }, 1000);
    return;
  }
  const action = event.target.id;
  const bettingAmount = round(
    Math.min(+$("#betting-amount").val(), +$("#betting-amount").attr("max")),
    0
  );
  ...
  if (bettingAmount > 0 && expiry > 0) {
    ...
    gameHistory.history.push({
      ...
      startingTimeStamp: new Date(
        displayedDataPoints[displayedDataPoints.length - 1][0]
      ),
      ...
      profit:
        action === "buy"
          ? +displayedDataPoints[displayedDataPoints.length - 1][4] <
            +remainingDataPoints[expiry - 1][4]
            ? bettingAmount
            : -bettingAmount
          : +displayedDataPoints[displayedDataPoints.length - 1][4] >
            +remainingDataPoints[expiry - 1][4]
    ...
    $("#game-history table tbody").prepend(`<tr id="bet-${
      gameHistory.history[gameHistory.history.length - 1].id
    }">
                                              <td>${
                                                gameHistory.history.length
                                              }</td>
                                              <td>${gameHistory.history[
                                                gameHistory.history.length - 1
                                              ].startingTimeStamp.toLocaleDateString(
                                                "en-US",
                                                dateOptions
                                              )}</td>
                                              <td>${numberToText(
                                              ...
                                              <td>${gameHistory.history[
                                                gameHistory.history.length - 1
                                              ].expiryTimeStamp.toLocaleDateString(
                                                "en-US",
                                                dateOptions
                                              )}</td>
                                              <td></td>
    ...
    if (betID <= 4) {
      $("#game-history table tbody tr").last().remove();
    }
    ...
    if (+$("#betting-amount").val() % 1 != 0) {
      $("#betting-amount").val(bettingAmount);
    } // If input value decimal -> replace with integer
    $("#betting-amount").blur();
  }
});
...
function drawChart() {
  ...
  $.getJSON(
    proxyURL +
      "https://data.nasdaq.com/api/v3/datasets/WIKI/" +
      asset +
      "/data.json?api_key=" +
      QUANDL_API_KEY,
    function (rawData) {
...
function generateDataArrays(data) {
  ...
  displayedDataPoints = [];
  remainingDataPoints = [];
  for (let i = 0; i < randomDisplayedIndex; i++)
    displayedDataPoints.push(data[i]);
  for (let i = randomDisplayedIndex; i < data.length; i++)
    remainingDataPoints.push(data[i]);
}
...
function checkForExpiredOpenPositions() {
  ...
  gameHistory.history.forEach((elem, index) => {
    if (elem.expiryIndex === currentIndex) expiredIndexArray.push(index);
  });
  if (expiredIndexArray.length > 0) {
    ...
    for (let i = 0; i < expiredIndexArray.length; i++) {
      ...
      closedPositionsAmount +=
        gameHistory.history[expiredIndexArray[i]].bettingAmount;
      closedPositionsProfit += gameHistory.history[expiredIndexArray[i]].profit;
      $("#bet-" + gameHistory.history[expiredIndexArray[i]].id + " td")
        .eq(-2)
        .text(
          numberToText(gameHistory.history[expiredIndexArray[i]].expiryPrice)
        );
      $("#bet-" + gameHistory.history[expiredIndexArray[i]].id + " td")
        .eq(-1)
        .text(numberToText(gameHistory.history[expiredIndexArray[i]].profit));
      if (gameHistory.history[expiredIndexArray[i]].profit > 0) {
        $("#bet-" + gameHistory.history[expiredIndexArray[i]].id).addClass(
          "success"
        );
        gameHistory.totalBetsWon++;
        gameHistory.totalAmountWon +=
          gameHistory.history[expiredIndexArray[i]].profit;
      } else {
        $("#bet-" + gameHistory.history[expiredIndexArray[i]].id).addClass(
          "danger"
        );
        gameHistory.totalAmountLost += Math.abs(
          gameHistory.history[expiredIndexArray[i]].profit
        );
      }
    }
    ...
    $("#win-rate").text(
      gameHistory.totalBetsWon + "/" + gameHistory.totalBetsMatured
    );
    if (gameHistory.totalBetsWon > 0) {
      $("#average-profit").text(
        numberToText(
          round(gameHistory.totalAmountWon / gameHistory.totalBetsWon, 0)
        )
      );
    }
    if (gameHistory.totalBetsMatured - gameHistory.totalBetsWon > 0) {
      $("#average-loss").text(
        numberToText(
          round(
            gameHistory.totalAmountLost /
              (gameHistory.totalBetsMatured - gameHistory.totalBetsWon),
            0
          )
        )
      );
    ...
    $("#total-profit").text(
      gameHistory.totalProfit >= 0
        ? "$" + numberToText(gameHistory.totalProfit)
        : "-$" + numberToText(Math.abs(gameHistory.totalProfit))
    );
    $("#betting-amount").attr("max", gameHistory.cashBalance);
    if (gameHistory.equity === 0) {
      // Game Over...
      if (isGameInProgress) {
        $("#start-end-game").click();
      }
